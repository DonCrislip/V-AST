...
import json from '../trade/trade.v-lac.json';
import Vue from 'vue';
const config = require(`../../v-lac.config.js`)
...
function ForceGraph({
  ...
  nodeId = d => d, // given d in nodes, returns a unique identifier (string)
...
//   const forceLink = d3.forceLink(links).id(({index: i}) => N[i].id);
...
//         .on('zoom', handleZoom)
    ...
    //     .attr('class', 'circles')
    //     .selectAll('g')
    //     .data(nodes.filter(o => o.type === 'module'))
    //     .enter().append('g').on('click', (e) => {
    //         vm.obj = e.target.__data__;
    //         node._groups.forEach(n => {
    //             n.forEach(g => g.classList.remove('selected'))
    //         })
    //         e.target.classList.add('selected')

    //         link._groups.forEach(n => {
    //             n.forEach(g => g.classList.remove('selected', 'source', 'target'))
    //         })
    //         const arr = links.filter(l => l.source.id === e.target.__data__.id || l.target.id === e.target.__data__.id);
    //         arr.forEach(l => {
    //             link._groups.forEach(n => {
    ...
    // const node = circleGroup.append('circle')
    //     .attr("fill", nodeFill)
    ...
    //     .selectAll('g')
    //     .data(nodes.filter(o => o.key))
    //     .enter().append('g')
        
    // const connector = connectorGroup.append('text')
    //     .attr('text-anchor', 'middle')
    //     .attr('class', 'module-name')
    //     .call(drag(simulation))
    //     .on('click', (e) => {
    ...
    // .selectAll('g')
    // .data(nodes.filter(o => o.type === 'endpoint'))
    // .enter().append('g')
    // .on('click', (e) => {
    //     vm.obj = e.target.__data__;
    //     node._groups.forEach(n => {
    ...
    // const service = serviceGroup.append('path')
    //     .attr("d", satelliteOutline)
    //     .attr('class', 'satellite')
    //     .call(drag(simulation))

    // const serviceName = serviceGroup.append('text')
    //     .attr('text-anchor', 'middle')
    //     .attr('class', 'module-name')

    // <path d="M317.02,48.09c-.64,.5-1.33,.95-1.9,1.52-33.17,33.15-66.32,66.32-99.48,99.48-.4,.4-.83,.76-1.34,1.23-6.15-6.21-12.21-12.34-18.19-18.38-3.66,3.75-7.14,7.33-10.7,10.97,8.72,8.65,17.75,17.61,26.88,26.66-5.47,5.31-10.62,10.31-16.05,15.58,2.79,2.6,5.07,4.82,7.48,6.9,.38,.33,1.48,.24,2-.07,9.5-5.76,19.69-8.09,30.71-6.21,2.12,.36,4.17,1.1,6.73,1.8-1.17,1.04-1.9,1.73-2.67,2.36-6.63,5.41-13.26,10.83-19.93,16.2-1.16,.93-1.77,1.9-1.49,3.45,.2,1.13,.14,2.31,.16,3.47,.03,3.38-2.7,6.35-6,5.75-4.25-.78-6.8,.96-9.2,4.13-4.44,5.84-9.15,11.48-13.75,17.2-.6,.74-1.22,1.46-2.28,2.72-.75-2.73-1.59-4.92-1.95-7.19-1.71-10.67,.47-20.61,6.1-29.79,.78-1.28,.85-2.22-.17-3.22-2.07-2.02-4.16-4.01-6.39-6.15-4.95,5.05-9.99,10.19-15.05,15.37-9.21-9.26-18.19-18.29-27.06-27.2-3.72,3.78-7.22,7.34-10.83,11,5.86,5.72,12.05,11.78,18.1,17.7-34.54,34.52-68.68,68.65-103.13,103.09-1.22-1.29-2.66-2.9-4.19-4.43-13.97-13.99-27.96-27.97-41.95-41.94-.44-.44-.97-.79-1.47-1.18,0-.13,0-.26,0-.39,9.3-9.27,18.62-18.53,27.91-27.82,21.51-21.49,43-43,64.5-64.5,3.37-3.37,6.73-6.75,10.2-10.23,5.9,5.76,11.72,11.45,17.92,17.52,3.56-3.63,6.88-7,10.49-10.68-10.99-11.12-21.71-21.96-32.56-32.93,4.04-3.87,8.06-7.73,12.16-11.65-1.44-1.65-2.47-2.86-3.52-4.05-.76-.86-.79-1.69,.05-2.53,4.96-4.96,9.92-9.92,14.87-14.9,.79-.79,1.59-.75,2.39-.09,1.23,1.03,2.43,2.09,3.94,3.38,3.85-4.05,7.69-8.09,11.72-12.33,3.53,3.56,6.71,6.84,9.95,10.04,7.2,7.1,14.44,14.16,21.62,21.28,1.11,1.1,1.87,1.15,3.01,.06,2.97-2.85,6.04-5.59,9.4-8.67-6.23-6.39-11.91-12.21-17.14-17.58C201,68.52,234.98,34.33,269.1,0c2.95,2.9,5.91,5.75,8.82,8.65,12.55,12.51,25.08,25.03,37.62,37.54,.43,.43,.98,.75,1.48,1.12v.77Zm-101.99,81.64c28.11-28.1,56.15-56.13,83.96-83.94-9.64-9.69-19.44-19.53-29.13-29.27-27.97,27.97-56.02,56.01-84.03,84.02,9.73,9.73,19.56,19.56,29.19,29.19Zm-111.76,54.94c-28.04,28.25-55.82,56.25-83.33,83.97,9.63,9.67,19.43,19.5,29.07,29.19,27.84-27.84,55.78-55.78,83.33-83.32-9.55-9.8-19.2-19.7-29.07-29.83Z"/><path d="M221.83,253.17c-1.41-.11-2.82-.18-4.22-.33-1.61-.18-2.41-1.09-2.27-2.75,.15-1.66,.36-3.3,2.5-3.53,1.92-.2,3.84-.38,5.77-.49,13.97-.76,24.03-12.45,22.74-26.46-.26-2.81,.22-3.69,2.27-4.13,3.17-.68,4.35,.26,4.65,3.56,1.64,17.51-12.21,34.59-31.45,34.13Z"/><path d="M219.55,236.29c-.58,0-1.16,.05-1.74,0-2.48-.25-3.24-.89-3.28-2.67-.06-2.65,.81-3.78,3.28-4.09,1.27-.16,2.59,0,3.84-.25,4.14-.82,7.39-4.48,8-8.89,.13-.96,.29-1.92,.27-2.88-.03-1.89,.69-2.88,2.7-2.88,2.44,0,3.61,.75,3.92,2.78,1.18,7.49-4.35,16.15-11.67,18.15-1.72,.47-3.53,.64-5.29,.95-.01-.07-.02-.14-.03-.22Z"/><path d="M215.04,129.73c-9.63-9.63-19.46-19.46-29.19-29.19,28.01-28.01,56.06-56.05,84.03-84.02,9.69,9.74,19.49,19.58,29.13,29.27-27.82,27.81-55.85,55.84-83.96,83.94Z" style="fill:#fff;"/><path d="M103.28,184.67c9.87,10.13,19.52,20.03,29.07,29.83-27.54,27.54-55.48,55.48-83.33,83.32-9.64-9.68-19.44-19.52-29.07-29.19,27.51-27.72,55.29-55.71,83.33-83.97Z" style="fill:#fff;"/>

//   if (W) link.attr("stroke-width", ({index: i}) => W[i]);
//   if (L) link.attr("stroke", ({index: i}) => L[i]);
//   if (S) node.attr("r", ({index: i}) => S[i]);
//   if (G) node.attr("fill", ({index: i}) => color(G[i]));
//   if (T) text.text(({index: i}) => T[i]);
//   if (T) connector.text(({index: i}) => T[i]);
//   if (T) serviceName.text(({index: i}) => T[i]);
...
//   d3.select('svg')
//     .call(zoom)
//     .call(zoom.transform, d3.zoomIdentity.scale(0.05))

    // ticked()

  return zoom//Object.assign(svg.node(), {scales: {color}});
}
...
for (const parent of json) {
    if (parent.name.includes('docImport')) continue;
    parent.show = true
...
const vm = new Vue({
    ...
    computed: {
        ...
        compData() {
            ...
            obj.nodes = this.nodes.filter(o => o.show)
            obj.links = this.links.filter(o => o.show)
            return obj
        },
    },
    methods: {
        ...
        chart() {
            ...
            this.nodes = d3.map(this.nodes, (_, i) => this.N[i]);
            this.links = d3.map(this.links, (_, i) => ({source: this.LS[i], target: this.LT[i], show: true}));
        ...
        handleEntryPointChange(entryPoint) {
            this.filterOnEntryPoint(this.nodes.find(o => o.name === entryPoint))
        },
