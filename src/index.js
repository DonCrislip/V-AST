...
import entrypoints from '../build/entrypoints.v-ast.js'
...
const vm = new Vue({
    ...
    data() {
        return {
            allData: this.parseData(entrypoints),
    ...
    methods: {
        ...
        parseData(data) {
            const allData = []
            for (const entryPoint of Object.keys(data)) {
                const json = data[entryPoint]
        ...
        addEntryPoint() {
            this.configEntryPoints.push({
                name: '',
                path: '',
                aliases: []
            })
        },
        removeObjFromArray(obj, arr) {
        ...
        updateConfig() {
            fetch('update', {
                ...
                body: JSON.stringify({
                    hasEntireCodeGalaxyOption: this.showEntireCodeGalaxy,
                    apiLiterals: this.apiLiterals,
                    entryPoints: this.configEntryPoints
                })
            }).then(async resp => {
                const res = await resp.json()
                this.allData = this.parseData(res)
                this.handleViews()
            }, error => {
                console.log(error)
            })
